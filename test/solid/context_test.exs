defmodule Solid.ContextTest do
  ...
  describe "get_in/3" do
    ...
    # Multi-scope priority tests — production uses [:iteration_vars, :vars, :counter_vars]
    ...
    test "default 3 scopes: counter_vars used as fallback" do
      var = %Variable{original_name: "x", loc: @loc, identifier: "x", accesses: []}
      context = %Context{iteration_vars: %{}, vars: %{}, counter_vars: %{"x" => "from_counter"}}
    ...
    test "default 3 scopes: not found in any scope" do
      var = %Variable{original_name: "x", loc: @loc, identifier: "x", accesses: []}
      context = %Context{iteration_vars: %{}, vars: %{}, counter_vars: %{}}

      assert Context.get_in(context, var, [:iteration_vars, :vars, :counter_vars]) ==
               {:error, {:not_found, ["x"]}, context}
    end

    test "nil in higher-priority scope does not shadow non-nil in lower-priority scope" do
      var = %Variable{original_name: "x", loc: @loc, identifier: "x", accesses: []}
    ...
    test "nil in vars does not shadow non-nil in counter_vars" do
      var = %Variable{original_name: "x", loc: @loc, identifier: "x", accesses: []}
      context = %Context{iteration_vars: %{}, vars: %{"x" => nil}, counter_vars: %{"x" => 42}}

      assert Context.get_in(context, var, [:iteration_vars, :vars, :counter_vars]) ==
               {:ok, 42, context}
    end

    test "false in higher-priority scope does shadow lower-priority scope" do
      var = %Variable{original_name: "x", loc: @loc, identifier: "x", accesses: []}
    ...
    test "nil is returned when it is the only value in any scope" do
      var = %Variable{original_name: "x", loc: @loc, identifier: "x", accesses: []}
      context = %Context{iteration_vars: %{"x" => nil}, vars: %{}, counter_vars: %{}}
